...
- **Chart:** forgejo-charts (OCI) `16.2.1` / Forgejo `14.0.3`
- **Config:** Rootless, mirror enabled (8h interval), actions enabled with 12h endless task timeout, OIDC via Authentik (temporarily disabled), SSH enabled (port 22), public `ROOT_URL` stays on `https://forgejo.${SECRET_DOMAIN}`, runner-local `LOCAL_ROOT_URL` points to `http://forgejo-ts.development.svc.cluster.local:3000`
